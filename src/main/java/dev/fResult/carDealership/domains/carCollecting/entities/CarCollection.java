...
import dev.fResult.carDealership.domains.carCollecting.valueObjects.Status;
...
public class CarCollection {
  ...
  @Embedded private VehicleRegistrationId vehicleRegistrationId;

  @Embedded private Coordinate place;

  @OneToMany private List<Pickup> pickups;
  ...
  public static CarCollection schedule(
    ...
    if (StringUtils.isBlank(contactName)) throw new ValidationException("Contact Name is required");
    if (StringUtils.isBlank(contactPhoneNumber))
      throw new ValidationException("Contact Phone Number is required");
    if (StringUtils.isBlank(address)) throw new ValidationException("Address is required");
    if (StringUtils.isBlank(carModel)) throw new ValidationException("Car Model is required");
    if (color == null) throw new ValidationException("Color is required");
    if (scheduleTime == null) throw new ValidationException("Schedule Time is required");
    if (vehicleRegistrationId == null)
      throw new ValidationException("Vehicle Registration Id is required");
    if (place == null) throw new ValidationException("Place is required");
  ...
  public void carDroppedToWarehouse() throws ValidationException {
    if (status != Status.Scheduled) throw new ValidationException("Car Collection already dropped");
  ...
  public void recordPickup(PickupResult result, String reason) throws ValidationException {
    ...
    if (status != Status.Scheduled)
      throw new ValidationException(
          "Car Collection is not scheduled for picking up. Cannot record pickup");
